...
const setPaginationState = () => {
    ...
    toggleNextBtn(appState.page, appState.total);//Botones de pasar páginas
  };

  // Función para renderizar el loader-muestra mientras se carga la pagina

const renderLoader = () => { //renderiza cargador
    return `
      <div class="lds-ring">
          <div></div>
          <div></div>
...
const togglePreviousBtn = (page) => {
  ...
  // Función para desabilitar o habilitar el boton siguiente del paginator. Si esta el cliente en la ultima pagina se deshabilita.
const toggleNextBtn = (page, total) => {
  ...
  // Función para crear el template de la card de las peliculas
//Funcion generadora
...
const formatVoteAverage = (voteAverage) => {
    return Math.round(voteAverage * 10); // 66.62 = 67 Redondear numeros
  };
...
const formatDate = (date) => {
    ...
    return `${day}/${month}/${year}`; //Modificar para que sea dia, mes y año
  };
...
const loadAndShow = (movies) => {
    setTimeout(() => { //Inventar tiempo de carga
      renderCards(movies.results);
      filterContainer.scrollIntoView({
        behavior: 'smooth',
      });
    }, 1500); //Minuto y medio de espera
  };

  // FUNCIONES PARA MANEJAR EL FILTRADO DE LAS PELICULAS en los botones principales
